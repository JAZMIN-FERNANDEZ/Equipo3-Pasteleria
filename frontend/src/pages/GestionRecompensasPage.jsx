        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input 
              ...
              id="nombrerecompensa" 
              value={formData.nombrerecompensa} 
          ...
          <div>
            <label htmlFor="puntosrequeridos" className="block text-sm font-medium text-gray-700">Condición (Gasto Mínimo $)</label>
            <input 
              type="number" 
...
function GestionRecompensasPage() {
  ...
  const handleOpenAddModal = () => { setSelectedRecompensa(null); setIsModalOpen(true); };
  const handleOpenEditModal = (recompensa) => { setSelectedRecompensa(recompensa); setIsModalOpen(true); };
  const handleCloseModal = () => { setIsModalOpen(false); setSelectedRecompensa(null); };
  ...
  const handleSaveRecompensa = async (formData, recompensaId) => {
    ...
    try {
      ...
      // Error manejado por interceptor (ej. validación fallida)
    }
  };
  ...
  if (loading) return <div className="text-center mt-10">Cargando reglas de recompensas...</div>;
          ...
          <table className="min-w-full divide-y divide-gray-200">
            <thead className="bg-gray-50">
              <tr>
                ...
                <th className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase">Condición (Gasto Mín)</th>
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {recompensas.map((r) => (
                <tr key={r.id_recompensa}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{r.tipo === 'PORCENTAJE_DESCUENTO' ? 'Descuento %' : 'Descuento Fijo'}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500 font-bold">
                    {r.tipo === 'PORCENTAJE_DESCUENTO' ? `${parseFloat(r.valor)}%` : `$${parseFloat(r.valor).toFixed(2)}`}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">${parseFloat(r.puntosrequeridos).toFixed(2)}</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium space-x-2">
                    <button onClick={() => handleOpenEditModal(r)} className="text-blue-600 hover:text-blue-800">Editar</button>
                    <button onClick={() => handleDeleteRecompensa(r.id_recompensa)} className="text-red-600 hover:text-red-800">Eliminar</button>
                  </td>
                </tr>
              ))}
            </tbody>
